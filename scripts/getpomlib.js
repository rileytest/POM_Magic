...
(function ($) {
    ...
    //lan_hd
    $.xpath = function (xpath, contentDocument) {
        if(contentDocument){
            return $(xp(xpath, contentDocument));
        }else{
            return $(xp(xpath, document));
        }
...
XpathLanLib = (function() {
    ...
    XpathLanLib.prototype = {
        ...
        getAttributesOther: function(element) {
        	...
        	for(i = 0; i < attributesLen; i = i + 1) {
        		try {
        			...
        			this.getLog("Error : XpathLanLib[getAttributesOther] : " + e);
        		}
            }
            return attributesName;
        },
        ...
        getAttributeCssById: function(element) {
            let resultValue = '';
            if(this.getAttributeValue(element, 'id') !== 0) {
                resultValue = '#' + CSS.escape(element.id);
            }
            return resultValue;
        },
        ...
        getAttributeCssByName: function(element) {
            let resultValue = '';
            if(this.getAttributeValue(element, 'name') !== 0) {
                resultValue = '[name="' + element.name + '"]';
            }
        ...
        getAttributeXpathByClassAndText: function(element) {
            ...
            try {
                ...
                let isExitText = this.hasTextValue(element);
                let contentText = $.trim(element.innerText);
                let tagName = element.tagName.toLowerCase();
                
                if(classValue !== 0 && isExitText) {
                    let subText = contentText.substring(0, 50);
                    if(tagName !== "input")
                        resultValue = '[contains(@class, \"' + classValue + '\") and contains(text(), \"' + subText + '\")]';
                    else
                        resultValue = '[contains(@class, \"' + classValue + '\")]';
                } else if(isExitText && tagName !== 'input') {
                    let subText = contentText.substring(0, 50);
                    resultValue = '[contains(text(), \"' + subText + '\")]';
                }
            } catch (e) {
        ...
        getAttributeCssByClassAndText: function(element) {
            let resultValue = '';
            try {
                let classValue = this.getAttributeValue(element, 'class'),
                    singleClass = '',
                ...
                //selenium2不支持css伪类，取消contains表达式
//                let isExitText = this.hasTextValue(element),
//                    contentText = $.trim(element.innerText),
//                    tagName = element.tagName.toLowerCase();
                ...
                if(classValue !== 0) {
                    singleClass = classValue.split(' ');
                    for(let i = 0; i < singleClass.length; i++) {
                        if(singleClass[i]) {
                            classStr += '.' + CSS.escape(singleClass[i]);
                        }
                    }
                }
                
                if(classStr !== '' && isExitText) {
                    let subText = contentText.substring(0, 50);
                    if(tagName !== "input")
                        resultValue = classStr + ':contains("' + subText + '")';
                    else
                        resultValue = classStr;
                } else if(isExitText && tagName !== 'input') {
                    let subText = contentText.substring(0, 50);
                    resultValue = ':contains("' + subText + '")';
                }
            } catch (e) {
                this.getLog("Error : XpathLanLib[getAttributeCssByClassAndText] : " + e);
            }
        ...
        getAttributeXpathByClassAndTitle: function(element) {
            ...
            if(classValue !== 0 && titleValue !== 0) {
                resultValue = '[contains(@class, \"' + classValue + '\") and @title=\"' + titleValue + '\"]';
            } else if(titleValue !== 0) {
                resultValue = '[@title=\"' + titleValue + '\"]';
            }
            
            return resultValue;
        },
        
        getAttributeCssByClassAndTitle: function(element) {
            let resultValue = '',
                classValue = this.getAttributeValue(element, 'class'),
                titleValue = this.getAttributeValue(element, 'title');
            ...
            try {
            	if(classValue !== 0) {
                    let singleClass = classValue.split(' ');
                    let classStr = '';
                    for(let i = 0; i < singleClass.length; i++) {
                        if(singleClass[i] && CSS.escape(singleClass[i]) == singleClass[i]) {
                            classStr += '.' + singleClass[i];
                        }
                    }
                    resultValue += classStr;
                }
                if(titleValue !== 0) resultValue += '[title="' + titleValue + '"]';
    		} catch (e) {
    			this.getLog("Error : XpathLanLib[getAttributeCssByClassAndTitle] : " + e);
    		}
        ...
        getAttributeCssByOtherAttribute: function(element) {
            let maxAttrLen = 3,
                otherAttributes = this.getAttributesOther(element),
                otherAttributesLen = otherAttributes.length,
                resultValue = '';
            ...
            for(let i = 0; i < maxAttrLen && i < otherAttributesLen; i++) {
            	if(CSS.escape(otherAttributes[i].name) == otherAttributes[i].name && CSS.escape(otherAttributes[i].value) == otherAttributes[i].value) {
            		resultValue += "[" + otherAttributes[i].name + '="' + otherAttributes[i].value + '"]';
            	}
            }
            if(resultValue !== '') {
        ...
        getXpathsById: function(element) {
            ...
            if(resultValue != "") {
                ...
                if(this.getResultsNumberAndTime(element.ownerDocument, xpath)) {
                    xpaths.push(xpath);
                }
            }
            return xpaths;
        ...
        getCssById: function(element) {
            let css = '',
                resultValue = '',
                cssArr = [];
            
            resultValue = this.getAttributeCssById(element);
            if(resultValue != '') {
                css = "css=" + element.tagName.toLowerCase() + resultValue;
                if(this.getResultsNumberAndTimeByCss(element.ownerDocument, css)) {
                    cssArr.push(css);
                }
            }
            return cssArr;
        ...
        getCssByName: function(element) {
            let css = '',
                resultValue = '',
                cssArr = [];
            
            resultValue = this.getAttributeCssByName(element);
            if(resultValue != '') {
                css = "css=" + element.tagName.toLowerCase() + resultValue;
                if(this.getResultsNumberAndTimeByCss(element.ownerDocument, css)) {
                    cssArr.push(css);
        ...
        getXpathsByText: function(element) {
            let xpath = "",
                resultVale = "",
                xpaths = [];
            
            resultVale = this.getAttributeXpathByName(element);
            if(resultVale != "") {
                xpath = "xpath=//" + element.tagName.toLowerCase() + resultVale;
                if(this.getResultsNumberAndTime(element.ownerDocument, xpath)) {
                    xpaths.push(xpath);
                }
            }
        ...
        getResultsNumberAndTime: function(doc, xpath) {
            ...
            if(finalXpath.substring(0, 8) == preXpath) {
                finalXpath = finalXpath.substr(preXpath.length - 2);
            }
            
            execEval = Xpath0rLib.getResultsNumberAndTime(doc, finalXpath);
        ...
        getResultsNumberAndTimeByCss: function(doc, cssStr) {
            
            let preCss = "css=",
                finalCss = cssStr;
            
            if(finalCss.substring(0, preCss.length) == preCss) {
                finalCss = finalCss.substr(preCss.length);
            }
            
            var result = $(finalCss, doc);
        ...
        generatorEvalXpath: function(element, preXpath) {
            let sufXpath = "",
                resultNameValue = "",
                resultTextValue = "",
                resultTitleValue = "",
...
//            }
            ...
            if(resultNameValue !== "") {
                ...
                if(this.getResultsNumberAndTime(element.ownerDocument, preXpath + sufXpath)) {
                    xpaths.push(preXpath + sufXpath);
                }
            }
        ...
        generatorEvalCss: function(element, preCss) {
            let sufCss = '',
                resultNameValue = '',
                resultTextValue = '',
                resultTitleValue = '',
...
//            let resultIdValue = this.getAttributeCssById(element);
//            if(resultIdValue !== '') {
//                sufCss = tagName + resultIdValue;
//                cssArr.push(preCss + sufCss);
//            }

            resultNameValue = this.getAttributeCssByName(element);
            if(resultNameValue !== '') {
                sufCss = tagName + resultNameValue;
                if(this.getResultsNumberAndTimeByCss(element.ownerDocument, preCss + sufCss)) {
                    cssArr.push(preCss + sufCss);
                }
            }

            resultTextValue = this.getAttributeCssByClassAndText(element);
            if(resultTextValue !== '') {
                sufCss = tagName + resultTextValue;
                if(this.getResultsNumberAndTimeByCss(element.ownerDocument, preCss + sufCss)) {
                    cssArr.push(preCss + sufCss);
                }
            }

            resultTitleValue = this.getAttributeCssByClassAndTitle(element);
            if(resultTitleValue !== '') {
                sufCss = tagName + resultTitleValue;
                if(this.getResultsNumberAndTimeByCss(element.ownerDocument, preCss + sufCss)) {
                    cssArr.push(preCss + sufCss);
                }
            }
            ...
            if(resultNameValue == '' && resultTextValue == '' && resultTitleValue == '') {
                let resultOtherValue = this.getAttributeCssByOtherAttribute(element);

                if(resultOtherValue !== '') {
                    sufCss = tagName + resultOtherValue;
                    if(this.getResultsNumberAndTimeByCss(element.ownerDocument, preCss + sufCss)) {
                        cssArr.push(preCss + sufCss);
                    }
                }
        ...
        getCssByAttributes: function(element) {
            let css = '',
                resultTextValue = '',
                resultTitleValue = '',
                tagName = element.tagName.toLowerCase(),
            ...
            resultTextValue = this.getAttributeCssByClassAndText(element);
            if(resultTextValue != '') {
                css = "css=" + tagName + resultTextValue;
                if(this.getResultsNumberAndTimeByCss(element.ownerDocument, css)) {
                    cssArr.push(css);
                }
            }
            ...
            resultTitleValue = this.getAttributeCssByClassAndTitle(element);
            if(resultTitleValue != '') {
                css = "css=" + tagName + resultTitleValue;
                if(this.getResultsNumberAndTimeByCss(element.ownerDocument, css)) {
                    cssArr.push(css);
                }
            }
            ...
            if(resultTextValue == '' && resultTitleValue == '') {
                let resultValue = this.getAttributeCssByOtherAttribute(element);
                if(resultValue !== '') {
                    css = "css=" + tagName + resultValue;
                    if(this.getResultsNumberAndTimeByCss(element.ownerDocument, css)) {
                        cssArr.push(css);
                    }
                }
        ...
        getXpathsByAncestorId: function(element) {
            ...
            for(let i = 0; i <= 6; i++) {
                ...
                if(this.getAttributeValue(currentElement, "id") !== 0) {
                    ...
                    xpaths = xpaths.concat(this.generatorEvalXpath(element, preXpath));
                }
            }
        ...
        getCssByAncestorId: function(element) {
            ...
            for(let i = 0; i <= 6; i++) {
                try {
                    ...
                    return cssArr;
                    this.getLog("Error : XpathLanLib[getCssByAncestorId] : " + e);
                }
                
                if(this.getAttributeValue(currentElement, 'id') !== 0) {
                    
                    let preCss = "css=" + currentElement.tagName.toLowerCase() + '#' + CSS.escape(currentElement.id) + ' ';
                    
                    cssArr = cssArr.concat(this.generatorEvalCss(element, preCss));
                }
            }
        ...
        getXpathsBySiblingId: function(element) {
            
            let currentElement = element,
                targetElement = null,
                nextSiblingXpath = "/following-sibling::",
                prevSiblingXpath = "/preceding-sibling::",
                preXpath = "",
                sufXpath = "",
                flag = false,
                xpaths = [];
            
            while(jQuery(currentElement).prev().size() > 0) {
                targetElement = jQuery(currentElement).prev().get(0);
                if(targetElement.hasAttribute('id')) {
                    flag = true;
                    preXpath = 'xpath=//' + targetElement.tagName.toLowerCase() + '[@id=\"' + targetElement.id + '\"]';
                    preXpath += nextSiblingXpath;
                    break;
                }
            ...
            if(!flag) {
                while(jQuery(currentElement).next().size() > 0) {
                    targetElement = jQuery(currentElement).next().get(0);
                    if(targetElement.hasAttribute('id')) {
                        flag = true;
                        preXpath = 'xpath=//' + targetElement.tagName.toLowerCase() + '[@id=\"' + targetElement.id + '\"]';
                        preXpath += prevSiblingXpath;
                        break;
                    }
                    currentElement = targetElement;
                }
        ...
        getCssBySiblingId: function(element) {
            ...
            if(jQuery(currentElement).prev().size() > 0) {
                targetElement = jQuery(currentElement).prev().get(0);
                if(targetElement.hasAttribute('id')) {
                    flag = true;
                    preCss = 'css=' + targetElement.tagName.toLowerCase() + this.getAttributeCssById(targetElement);
                    preCss += nextSiblingCss;
                }
            }
        ...
        getXpathsBySiblingText: function(element) {
            ...
            while(jQuery(currentElement).next().size() > 0) {
                ...
                if(targetElement.textContent != '') {
                    flag = true;
                    let nodetext = targetElement.textContent.substring(0, 50);
                    preXpath = 'xpath=//' + targetElement.tagName.toLowerCase() + '[contains(text(), \"' + nodetext + '\")]';
                    preXpath += prevSiblingXpath;
                    break;
                }
                currentElement = targetElement;
        ...
        getCssBySiblingText: function(element) {
            ...
            if(jQuery(currentElement).prev().size() > 0) {
                ...
                if(targetElement.textContent != '') {
                    flag = true;
                    preCss = 'css=' + targetElement.tagName.toLowerCase() + this.getAttributeCssByClassAndText(targetElement);
                    preCss += nextSiblingCss;
                }
            }
        ...
        parseCss20rXpath: function(cssArr) {
            let arrLen = cssArr.length,
                i = 0,
                css0rArray = [];
            
            for(i = 0; i < arrLen; i++) {
                css0rArray.push({css: cssArr[i].toString(), score: 100});
            }
            return css0rArray;
        },
...
//            xpaths.push(this.getXpathsByName(element));
            ...
            xpaths = xpaths.concat(this.getXpathsBySiblingId(element));
            
            if(xpaths.length == 0) {
                xpaths = xpaths.concat(this.getXpathsBySiblingText(element));
            }
            
            return this.parseXpath20rXpath(xpaths);
        },
        
        getCssArray: function(element) {
            let cssArr = [];
            
//            cssArr.push(this.getCssById(element));
//            cssArr.push(this.getCssByName(element));
            cssArr = cssArr.concat(this.getCssByAttributes(element));
            cssArr = cssArr.concat(this.getCssByAncestorId(element));
            cssArr = cssArr.concat(this.getCssBySiblingId(element));
            
            if(cssArr.length == 0) {
                cssArr = cssArr.concat(this.getCssBySiblingText(element));
            }
            
            return this.parseCss20rXpath(cssArr);
        }
...
var GetPomLib = GetPomLib || {};

GetPomLib = (function() {
    
    function GetPomLib() {
        this.filterElements = new Array();
        this.filterElements.push("<div eno=\"resize\" class=\"g-resize disabled\"></div>");
        this.filterElements.push("<div></div>");
    }
    ...
    GetPomLib.prototype = {
        ...
        filterElements: [], //过滤元素，抓取时过滤，不包含在抓取结果中
        
        getNewMyElementObj: function() {
        ...
        getNewMyElementContainer: function(mapvalue, actionname, priorvalue) {
            
            let MyElement = {
                mapValue		    : mapvalue,     //
                actionName			: actionname,   //array,some action for the element has,undefined
                prior				: priorvalue,   //优先级
                id					: "",	        //HTMLֵ
                name				: "",           //HTMLֵ
                value				: "",	        //HTMLֵ
                title				: "",	        //HTMLֵ
                xpathStr			: "",	        //xpath
                relativeXpathStr	: "",	        //xpath
                xpaths              : [],           //xpaths
                cssStr				: "",	        //css
                cssArr              : [],           //css array
                tag					: "",	        //element tag,eg:input
                type				: "",	        //element type,eg:button
        ...
        getElementTreeXPath: function (element)
        {
            ...
            for (; element && element.nodeType == 1; element = element.parentNode)
            {
                ...
                var pathIndex = (index || flag ? "[" + (index+1) + "]" : "");
        ...
        //Gets an relative XPath for an element.
        getElementRelativeXPath: function (element)
        {
            try
            {
                if (element && element.id)
                    ...
                    return this.getElementRelativeTreeXPath(element);
            }
            catch(E)
            {
                // xxxpedro: trying to detect the mysterious error:
        ...
        getElementRelativeTreeXPath: function (element)
        {
            var paths = [];
            ...
            for (; element && element.nodeType == 1; element = element.parentNode)
            {
                ...
                if(element.id){
                    paths.splice(0, 0, '//' + nodeName.toLowerCase() +'[@id=\"' + element.id + '\"]');
                    return paths.length ? paths.join("/") : null;
                }
            ...
            return paths.length ? "/" + paths.join("/") : null;	//if no nodes has node's id,return full path;
        },
        ...
        getIframePath: function(theWindow) {
            //获取iframe路径
            let iframePath = "",
                iframes = [],
                iframesLen = 0,
            ...
            while(currentWindow != window.top) {
                try {
                    iframes = jQuery("iframe, frame", currentWindow.parent.document).toArray();
                    iframesLen = iframes.length;
                    iframesIdx = 0;
                    for(; iframesIdx < iframesLen; iframesIdx++) {
                        if(jQuery(iframes[iframesIdx].contentDocument).is(currentWindow.document)) {

                            let $this = jQuery(iframes[iframesIdx]);
                            iframeNameOrId = $this.attr("id") || $this.attr("name") || "";

                            if(! iframeNameOrId) {
                                iframeNameOrId = iframesIdx;
                            }
                            break;
                        }
                    }
                    ...
                    iframeNameOrId = "<" + currentWindow.document.URL + ">";
                    if(iframePath) {
                        iframePath = iframeNameOrId + "/" + iframePath;
                    } else {
                        iframePath = iframeNameOrId;
        ...
        getPOM: function() {
            ...
            try {
                ...
                if(clickedEl){
                    ...
                    getElement(clickedEl, null, this.pomElements);

                    this.pomElements[0].frameId = this.getIframePath(window.self);

                    var request = {
                        'topic': 'getSingleElement',
                        'elements': this.pomElements
                      };
